...
class JsonRpcHttpClientTransportTest {
  ...
  @Test
  fun sendMessage_withInterceptor_addsReturnedHeadersToRequest() {
    enqueueTaskResponse()
    val interceptor = RecordingInterceptor("secret-token")
    val transport =
      JsonRpcHttpClientTransport(
        AndroidA2AHttpClient(),
        server.url("/a2a").toString(),
        agentCard(),
        listOf(interceptor),
      )
    ...
    assertThat(interceptor.capturedMethod).isEqualTo("message/send")
  }

  @Test
  fun sendMessage_withContextHeaders_seedsInterceptorAndRequest() {
    enqueueTaskResponse()
    val interceptor = RecordingInterceptor("token")
    val transport =
      JsonRpcHttpClientTransport(
        AndroidA2AHttpClient(),
        server.url("/a2a").toString(),
    ...
    val context = ClientCallContext(emptyMap(), mapOf("X-From-Context" to "ctx-value"))

    assertThat(transport.sendMessage(sendMessageParams(), context)).isNotNull()

    // The interceptor receives the context headers as its seed input...
    assertThat(interceptor.capturedHeaders).containsEntry("X-From-Context", "ctx-value")
    // ...and both the seed and interceptor-added headers reach the wire.
    val recorded = server.takeRequest()
    assertThat(recorded.getHeader("X-From-Context")).isEqualTo("ctx-value")
    assertThat(recorded.getHeader("Authorization")).isEqualTo("Bearer token")
  }

  private fun enqueueTaskResponse() {
    val task =
      Task.builder()
        .id("task-1")
        .contextId("context-1")
  ...
  /** Test interceptor that records its inputs and appends an `Authorization` header. */
  private class RecordingInterceptor(private val token: String) : ClientCallInterceptor() {
    ...
    override fun intercept(
      ...
      return PayloadAndHeaders(payload, headers + ("Authorization" to "Bearer $token"))
    }
  }
}
