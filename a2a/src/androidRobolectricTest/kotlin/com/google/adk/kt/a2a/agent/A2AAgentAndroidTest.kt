...
class A2AAgentAndroidTest {
  ...
  @Test
  fun runAsync_streamingCapableCard_usesNonStreamingWithoutCrashing() = runTest {
    // Regression: a card advertising streaming used to route the SDK Client to
    // `sendMessageStreaming`, which the proto-free Android transport does not implement (it threw
    // UnsupportedOperationException). The Android agent now forces non-streaming, so `message/send`
    // is used and the round-trip completes.
    val agentReply = "Hello from a streaming-capable agent!"
    val agentMessage =
      Message.builder()
        .messageId("agent-message-1")
        .role(Message.Role.ROLE_AGENT)
    ...
    val agentCard =
      AgentCard.builder()
        ...
        // Streaming-capable card: the case that previously crashed the Android transport.
    ...
    // Would throw UnsupportedOperationException before the fix; now completes over `message/send`.
    val events = agent.runAsync(context).toList()

    val emittedTexts = events.mapNotNull { it.content?.parts?.firstOrNull()?.text }
    assertThat(emittedTexts).contains(agentReply)
    assertThat(server.takeRecordedRequestOrFail().path).isEqualTo("/a2a")
  }
}
