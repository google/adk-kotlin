...
private const val APP_NAME = "rag-memory-example"
...
/**
 * A standard-ADK example of using [VertexAiRagMemoryService] with an agent.
 *
 ...
 *
 * This mirrors the "Memory Bank quickstart" agent shape from the Python ADK samples, but backed by
 * the RAG memory service.
 *
 * Vertex AI RAG indexes uploaded files asynchronously, so what one turn stores becomes searchable a
 * little later; run the example again to see the agent recall it in a fresh session.
 *
 ...
 * - `short` - a greeting that states no preferences. Run this after indexing: any preference the
 *   model uses must have been recalled, which the `[memory] injected past conversations: true` line
 *   confirms.
 *
 ...
 * - `VERTEX_RAG_CORPUS` - the bare RAG corpus id.
 * - `GOOGLE_CLOUD_LOCATION` - Vertex region for the corpus (optional, defaults to `us-central1`).
...
fun main(args: Array<String>) {
  runBlocking {
    ...
    val ragCorpus = requireEnv("VERTEX_RAG_CORPUS")
    val location =
    ...
    val memoryService =
      VertexAiRagMemoryService(project = project, location = location, ragCorpus = ragCorpus)
    ...
    // The first argument picks the user message: "short" sends only the greeting (no preferences),
    // so any preference the model brings up must have been recalled from memory - the easy way to
    // check recall. Any other value (default) sends the full message that seeds those preferences.
    val message =
    ...
    println(
      "\nThe after-agent callback stored this turn to the RAG corpus. Run the example again " +
        "(after indexing) with `short` argument to watch PreloadMemoryTool recall it in a new " +
        "session."
    )
  }
