...
import com.google.adk.kt.agents.BlockingAgent;
...
public final class SequentialAgentDemoJava {
  ...
  private static final class DummyStoryAgent extends BlockingAgent {
    ...
    @Override
    public List<Event> runBlocking(InvocationContext context) {
      return List.of(
          Event.builder()
              .invocationId(context.getInvocationId())
              .author(getName())
              .content(
                  new Content(/* role= */ null, List.of(Part.builder().text(storyText).build())))
