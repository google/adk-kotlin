...
import com.google.adk.kt.runners.BlockingRunner;
import com.google.adk.kt.runners.InMemoryRunner;
import com.google.adk.kt.sessions.BlockingSessionService;
...
public final class VertexAiSessionServiceExampleJava {
  ...
  /** A single tool the agent can call, exposed to the model via its declaration. */
  private static final class GetWeatherTool extends BlockingTool {
  ...
  public static void main(String[] args) {
    ...
    VertexAiSessionService sessionService =
        VertexAiSessionService.builder()
            ...
            .build();
    BlockingSessionService sessions = new BlockingSessionService(sessionService);
    ...
    String sessionId =
        Objects.requireNonNull(
            sessions.createSession(new SessionKey(appName, userId, null)).getKey().getId());
    ...
    Session session = sessions.getSession(new SessionKey(appName, userId, sessionId));
    System.out.println(
        "Session "
            + sessionId
            + " now has "
