...
object AndroidA2AAgentDemo {
  ...
  val rootAgent = run {
    ...
    // The Android transport is non-streaming, so the Client uses `message/send`, not SSE
    // `message/stream`. `runBlocking` keeps the sample simple; real apps fetch off the main thread.
    runBlocking { AndroidA2AAgent(name = agentName, agentCardUrl = agentUrl) }
  }
}
