...
/**
 * Java port of the standard-ADK example of using {@link VertexAiRagMemoryService} with an agent.
 *
 ...
 *   <li>{@code VERTEX_RAG_CORPUS} - the bare RAG corpus id.
 *   <li>{@code GOOGLE_CLOUD_LOCATION} - Vertex region for the corpus (optional, defaults to {@code
 *       us-central1}).
 *   <li>{@code GOOGLE_CLOUD_MODEL_LOCATION} - Vertex region for the model (optional, defaults to
 *       {@code global}).
 * </ul>
 */
public final class VertexAiRagMemoryServiceExampleJava {

  private static final String MODEL_NAME = "gemini-3.1-flash-lite";
  private static final String APP_NAME = "rag-memory-example";
  ...
  public static void main(String[] args) {
    ...
    String ragCorpus = requireEnv("VERTEX_RAG_CORPUS");
    ...
    MemoryService memoryService =
        VertexAiRagMemoryService.builder()
            .project(project)
            .location(location)
            .ragCorpus(ragCorpus)
            .build();
    ...
    System.out.println(
        "\n"
            ...
            + " (after indexing) with the `short` argument to watch PreloadMemoryTool recall it in"
            + " a new session.");
    runner.close();
    System.exit(0);
  }
  ...
  private VertexAiRagMemoryServiceExampleJava() {}
}
