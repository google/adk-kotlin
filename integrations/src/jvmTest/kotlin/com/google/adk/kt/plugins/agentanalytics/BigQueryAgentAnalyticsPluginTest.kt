...
class BigQueryAgentAnalyticsPluginTest {
  ...
  @Test
  fun redactTree_redactsHyphenatedSensitiveKeys() {
    val redacted = JsonFormatter.redactTree(mapOf("API-Key" to "secret-123", "id-token" to "t"))
    assertEquals(JsonFormatter.REDACTED_MESSAGE, redacted.get("API-Key").asText())
    assertEquals(JsonFormatter.REDACTED_MESSAGE, redacted.get("id-token").asText())
  }

  @Test
  fun redactSensitivePatterns_redactsCredentialsEmbeddedInText() {
    assertEquals(
      "https://example.com/o?api-key=[REDACTED]&alt=media",
      JsonFormatter.redactSensitivePatterns("https://example.com/o?api-key=abc123xyz&alt=media"),
    )
    assertEquals(
      "Authorization: [REDACTED]",
      JsonFormatter.redactSensitivePatterns("Authorization: Bearer abc.def.ghi"),
    )
    assertEquals(
      "call failed with bearer [REDACTED]",
      JsonFormatter.redactSensitivePatterns("call failed with bearer abc.def.ghi"),
    )
    assertEquals(
      """{"access_token": "[REDACTED]"}""",
      JsonFormatter.redactSensitivePatterns("""{"access_token": "ya29.abcdef"}"""),
    )
  }

  @Test
  fun redactSensitivePatterns_leavesOrdinaryProseUnchanged() {
    val prose = "[INFO] the bearer of this note may retry; count=3, ratio=0.5"
    assertEquals(prose, JsonFormatter.redactSensitivePatterns(prose))
  }

  @Test
  fun redactSensitivePatterns_isIdempotent() {
    val once = JsonFormatter.redactSensitivePatterns("api_key=abc123")
    assertEquals(once, JsonFormatter.redactSensitivePatterns(once))
  }

  @Test
  @Suppress("CheckReturnValue")
  fun logEvent_stampsEventIdAndAdkEnvelope(): Unit = runBlocking {
    whenever(mockBigQuery.getTable(any<TableId>())).thenReturn(mockTable)
    whenever(mockBigQuery.insertAll(any<InsertAllRequest>())).thenReturn(mockInsertAllResponse)
    whenever(mockInsertAllResponse.hasErrors()).thenReturn(false)
    ...
    verify(mockBigQuery)
      .insertAll(
        check<InsertAllRequest> { request ->
          val row = request.rows.single().content
          assertNotNull(UUID.fromString(row["event_id"] as String))
          val attributes = JsonFormatter.mapper.readTree(row["attributes"] as String)
          assertEquals(
            BigQuerySchema.ADK_ENVELOPE_SCHEMA_VERSION,
            attributes.get(BigQuerySchema.ADK_ATTRIBUTE_KEY).get("schema_version").asText(),
          )
        }
      )
  }
  ...
  @Test
  @Suppress("CheckReturnValue")
  fun logEvent_assignsADistinctEventIdPerRow(): Unit = runBlocking {
    whenever(mockBigQuery.getTable(any<TableId>())).thenReturn(mockTable)
    whenever(mockBigQuery.insertAll(any<InsertAllRequest>())).thenReturn(mockInsertAllResponse)
    whenever(mockInsertAllResponse.hasErrors()).thenReturn(false)
    ...
    verify(mockBigQuery, times(2))
      .insertAll(
        check<InsertAllRequest> { request ->
          eventIds.add(request.rows.single().content["event_id"] as String)
        }
      )
  ...
  fun schema_hasCorrectFields() {
    ...
    val eventIdField = fields.find { it.name == "event_id" }
    assertNotNull(eventIdField)
    assertEquals(StandardSQLTypeName.STRING, eventIdField.type.standardType)
    assertEquals(Field.Mode.NULLABLE, eventIdField.mode)
