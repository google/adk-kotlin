...
internal fun Application.adkApiModule(config: AdkServerConfig, webUiEnabled: Boolean) {
  ...
  routing {
    ...
    if (!camelCase) {
      logger.warn(
        "Responses use the pre-contract spelling: /version emits `language_version` and the " +
          "development trace endpoints emit `span_id`. The agent runtime contract asks for " +
          "camelCase, and this default will change in a future release; set camelCaseEnforced " +
          "or {}=true to move now.",
...
/** Property that switches responses to the contract's camelCase spelling. */
internal const val CAMEL_CASE_ENFORCED_PROPERTY = "adk.wire.camelcase.enforced"

/**
 * Whether responses use the contract's camelCase spelling: the property first, then the Ktor
 * config, then [AdkServerConfig], else the pre-contract spelling so the Development UI keeps
 * working.
 *
