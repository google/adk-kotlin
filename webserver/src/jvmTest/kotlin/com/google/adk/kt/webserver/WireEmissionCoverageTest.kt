 ...
 * rather than by scanning response text, which does not. [WireContractTest] exercises the same rule
...
class WireEmissionCoverageTest {
  ...
  /**
   * The types the contract endpoints emit: `/version`, the session routes, the artifact routes, and
   * `/run` and `/run_sse`. `/health` and `/list-apps` emit no schema of their own.
   */
