...
// This compilation builds the fixtures but is not expected to run the suite, which needs a
// manifest registering the generated `<service>`.
dependencies { add("kspAndroidDeviceTest", libs.androidx.appfunctions.compiler) }

// The AppFunctions compiler aggregates the discovered functions into a single generated service and
// XML. This arg is read only by that compiler, which runs only in kspAndroidDeviceTest, so it is
// inert in the Room and @Tool KSP passes.
