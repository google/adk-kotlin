...
class McpToolsetCoreTest {
  @Test
  fun sessionManagerDefaultDoesNotApplyAHeaderSnapshot() = runBlocking {
    val manager: McpClientSessionManager = FakeSessionManager(FakeSession())

    manager.getSession()

    assertNull((manager as FakeSessionManager).headersSeen.single())
  }
  ...
  @Test
  fun toolExecution_serverRejection_returnsErrorWithoutRetrying() = runBlocking {
    val session =
      FakeSession(rejectToolCall = McpServerRejectedException("Invalid arguments for tool", jsonRpcCode = -32602))
    val manager = FakeSessionManager(session, FakeSession())
    val tool = McpToolsetCore(manager).getTools().single()

    val result = tool.run(testToolContext(), mapOf("a" to 1))

    assertThat((result as Map<*, *>)["error"].toString()).contains("Invalid arguments for tool")
    assertThat(session.callToolCalls).isEqualTo(1)
    assertThat(manager.staleSessions).isEmpty()
    Unit
  }

  @Test
  fun listResources_serverRejection_returnsErrorWithoutRetrying() = runBlocking {
    val session =
      FakeSession(
        supportsResources = true,
        rejectListResources = McpServerRejectedException("Invalid cursor", jsonRpcCode = -32602),
      )
    val manager = FakeSessionManager(session, FakeSession(supportsResources = true))
    val tool =
      McpToolsetCore(manager, useMcpResources = true).getTools().single { it.name == "list_mcp_resources" }
    ...
    assertThat(session.listResourceCalls).isEqualTo(1)
    assertThat(manager.staleSessions).isEmpty()
    Unit
  }

  @Test
  fun listResourceTemplates_serverRejection_returnsErrorWithoutRetrying() = runBlocking {
    val session =
      FakeSession(
        supportsResources = true,
        rejectListTemplates = McpServerRejectedException("Invalid cursor", jsonRpcCode = -32602),
      )
    val manager = FakeSessionManager(session, FakeSession(supportsResources = true))
    val tool =
      McpToolsetCore(manager, useMcpResources = true)
        .getTools()
        .single { it.name == "list_mcp_resource_templates" }

    val result = tool.run(testToolContext(), mapOf("cursor" to "stale-cursor"))

    assertThat((result as Map<*, *>)["error"].toString()).contains("Invalid cursor")
    assertThat(session.listTemplateCalls).isEqualTo(1)
    assertThat(manager.staleSessions).isEmpty()
    Unit
  }

  @Test
  fun loadResource_nameAndRejectedListing_returnsMessageWithoutRetrying() = runBlocking {
    val session =
      FakeSession(
        supportsResources = true,
        rejectListResources = McpServerRejectedException("Listing refused", jsonRpcCode = -32602),
      )
    val manager = FakeSessionManager(session, FakeSession(supportsResources = true))
    val tool =
      McpToolsetCore(manager, useMcpResources = true).getTools().single { it.name == "load_mcp_resource" }

    val result = tool.run(testToolContext(), mapOf("name" to "greeting"))

    assertThat(result.toString()).contains("Listing refused")
    assertThat(session.listResourceCalls).isEqualTo(1)
    assertThat(session.readResourceCalls).isEqualTo(0)
    Unit
  }

  @Test
  fun loadResource_uriRejected_returnsMessageWithoutRetrying() = runBlocking {
    val session =
      FakeSession(
        supportsResources = true,
        rejectRead = McpServerRejectedException("Unknown resource: mem://doc/nope", jsonRpcCode = -32602),
      )
    val manager = FakeSessionManager(session, FakeSession(supportsResources = true))
    val tool =
      McpToolsetCore(manager, useMcpResources = true).getTools().single { it.name == "load_mcp_resource" }
  ...
  fun sharedResourceTool_wrapsTransportFailuresInThePublicExecutionException() = runBlocking {
    ...
    assertThat(error.message).isEqualTo("Failed to load MCP resource.")
    assertThat(generateSequence(error.cause) { it.cause }.map { it.message }.toList())
  ...
  private class FakeSession(
    ...
    failToolCalls: Int = 0,
    private val rejectToolCall: McpServerRejectedException? = null,
    private val rejectListResources: McpServerRejectedException? = null,
    private val rejectListTemplates: McpServerRejectedException? = null,
    private val rejectRead: McpServerRejectedException? = null,
    ...
    override suspend fun listResources(cursor: String?): McpClientResourcePage {
      ...
      rejectListResources?.let { throw it }
    ...
    override suspend fun listResourceTemplates(cursor: String?): McpClientResourceTemplatePage {
      ...
      rejectListTemplates?.let { throw it }
