...
internal class ListMcpResourcesCoreTool(private val toolset: McpToolsetCore) :
  ...
  override suspend fun run(context: ToolContext, args: Map<String, Any?>): Any = try {
    ...
    throw error
  } catch (error: McpServerRejectedException) {
    logger.warn { "MCP server rejected a resource listing with code ${error.jsonRpcCode}." }
    mapOf("error" to "The MCP server rejected the listing: ${error.message}")
  } catch (error: Exception) {
    throw McpToolException.McpToolExecutionException("Failed to list MCP resources.", error)
  }

  override fun declaration(): FunctionDeclaration = cursorDeclaration(name, description)

  private companion object {
    val logger = LoggerFactory.getLogger(ListMcpResourcesCoreTool::class)
...
internal class ListMcpResourceTemplatesCoreTool(private val toolset: McpToolsetCore) :
  ...
  override suspend fun run(context: ToolContext, args: Map<String, Any?>): Any = try {
    ...
    logger.warn {
      "MCP server rejected a resource template listing with code ${error.jsonRpcCode}."
    }
    mapOf("error" to "The MCP server rejected the listing: ${error.message}")
  } catch (error: Exception) {
  ...
  private companion object {
    val logger = LoggerFactory.getLogger(ListMcpResourceTemplatesCoreTool::class)
    const val DESCRIPTION =
...
internal class LoadMcpResourceCoreTool(
  ...
  override suspend fun run(context: ToolContext, args: Map<String, Any?>): Any {
    ...
    return try {
      ...
      if (key != URI && requestedUri == value) {
        logger.warn {
          "MCP server rejected a resource listing with code ${error.jsonRpcCode}."
        }
        listingRejectedMessage(error.message ?: "no reason given")
      } else {
        logger.warn {
          "MCP server rejected a resource read with code ${error.jsonRpcCode}."
        }
        uriRejectedMessage(requestedUri, error.message ?: "no reason given")
      }
    } catch (error: McpResourceNotFoundException) {
      uriNotFoundMessage(requestedUri)
    } catch (error: Exception) {
      throw McpToolException.McpToolExecutionException("Failed to load MCP resource.", error)
    }
  }
  ...
  private companion object {
    ...
    val logger = LoggerFactory.getLogger(LoadMcpResourceCoreTool::class)
    const val DESCRIPTION =
