  ...
  private suspend fun <T> resourceCall(
    ...
    repeat(LOAD_RETRY_COUNT) { attempt ->
      ...
      try {
        ...
        // The server answered and rejected the request, so retrying repeats an identical round
        // trip and evicting punishes everyone sharing the session.
        throw error
      } catch (error: Exception) {
        if (attempt + 1 == LOAD_RETRY_COUNT) {
  ...
  private suspend fun callTool(
    ...
    repeat(CALL_RETRY_COUNT) { attempt ->
      ...
      try {
        ...
        logger.warn {
          "MCP server rejected a tool call with code ${error.jsonRpcCode}."
        }
        return mapOf("error" to "MCP tool execution failed: ${error.message}")
      } catch (error: Exception) {
