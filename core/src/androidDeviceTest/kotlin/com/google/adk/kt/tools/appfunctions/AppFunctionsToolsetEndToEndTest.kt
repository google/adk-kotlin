...
class AppFunctionsToolsetEndToEndTest {
  ...
  fun run_argumentOfTheWrongType_isRejectedBeforeReachingTheApp() = runBlocking {
    ...
    assertThat(error).isEqualTo("Parameter 'pages' expects a 32-bit whole number")
  }

  @Test
  fun run_functionReturningAScreen_handsThePendingIntentToTheApp() = runBlocking {
    var handed: PendingIntent? = null
    val withHandler = AppFunctionsToolset(context, onUiAction = { handed = it })
    val showNote = awaitTool("showNote", withHandler)

    val result = showNote.run(testToolContext(), mapOf("noteId" to "1"))

    // The intent is the whole result of such a function; dropping it would look like success.
    assertThat(handed).isNotNull()
    // Pins it to this APK, where merely being non-null would pass on any intent at all. The lint
    // bans this as a security mechanism; here it is the assertion itself, deciding nothing.
    @Suppress("PendingIntentCreator")
    assertThat(handed!!.creatorPackage).isEqualTo(context.packageName)
    // Spelled out because the converter that owns this key is not visible from here.
    assertThat(result).isEqualTo(mapOf(BaseTool.RESULT_KEY to mapOf("uiAction" to true)))
  }

  @Test
  fun run_functionReturningAScreen_withNoHandler_saysItCannotBeShown() = runBlocking {
    val showNote = awaitTool("showNote")
   ...
   *
   * @param from the toolset to poll, so a test using its own handler waits the same way.
   */
  private suspend fun awaitTool(suffix: String, from: AppFunctionsToolset = toolset): BaseTool =
    checkNotNull(
      withTimeoutOrNull(INDEXING_TIMEOUT) {
        var found: BaseTool? = null
        while (found == null) {
          found = from.getTools().firstOrNull { it.name.endsWith(suffix) }
      ...
      "No app function ending in '$suffix' was discovered within $INDEXING_TIMEOUT, on a device " +
        "that supports AppFunctions. Discovered: ${from.getTools().map { it.name }}"
    }
