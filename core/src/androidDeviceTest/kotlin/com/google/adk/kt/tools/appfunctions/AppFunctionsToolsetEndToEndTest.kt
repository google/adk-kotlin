 ...
 * Below Android 16 an absent `AppFunctionManager` is legitimate and the tests skip; from 16 on it
 * is a regression and they fail. The APK registers itself as an AppFunctions provider, so no
...
class AppFunctionsToolsetEndToEndTest {
  ...
  @Before
  fun requireAppFunctionsWhereTheyShouldExist() {
    ...
    // Below Android 16 an absent manager is legitimate, since 34 and 35 need the extension
    // library. From 16 on it is a regression, and skipping would report that green.
    assumeTrue(
      "This device does not support AppFunctions; skipping the end-to-end test.",
      Build.VERSION.SDK_INT >= Build.VERSION_CODES.BAKLAVA || manager != null,
    )
  ...
  fun getTools_discoversTheFunctionsThisAppDeclares() = runBlocking {
    ...
    // One configured package, so nothing has to be told apart and the name the model sees is the
    // app's own method name.
    assertThat(names.filter { it == "createNote" }).hasSize(1)
    assertThat(names.filter { it == "echo" }).hasSize(1)
  ...
  @Test
  fun run_serializableParameter_isDeliveredToTheApp() = runBlocking {
    val fileNote = awaitTool("fileNote")

    val result =
      fileNote.run(testToolContext(), mapOf("note" to mapOf("title" to "Groceries", "pages" to 3)))
