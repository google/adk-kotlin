...
internal object AppFunctionDataConverter {
  ...
  fun toAppFunctionData(metadata: AppFunctionMetadata, args: Map<String, Any?>): AppFunctionData {
    ...
    // No catch around this: `build()` only rejects a required property that was never set, and the
    // check above already refuses that -- with the same `isRequired && !isNullable` predicate the
    // spec uses, and additionally for a parameter named `id`, which `containsKey` always reports
    // present.
  ...
  /**
   * The screen a function returned for the caller to open, or `null` when it returned data.
   *
   * A `PendingIntent` is not a value the model can read, so it is fetched separately rather than
   * through [fromReturnValue].
   */
  fun pendingIntentReturn(
    metadata: AppFunctionMetadata,
    returnValue: AppFunctionData,
  ): PendingIntent? {
    if (!AppFunctionTypes.isPendingIntent(metadata.response.valueType, metadata.components)) {
      return null
    }
    return returnValue.getParcelable(RETURN_VALUE_KEY, PendingIntent::class.java)
  }
   ...
   *
   * A numeric getter substitutes a default for an absent value. [AppFunctionData.containsKey] would
   * tell an absent value from a set one for every key except the reserved `id`, which it always
   * reports present; the double-probe is used so one path covers that case too.
   */
