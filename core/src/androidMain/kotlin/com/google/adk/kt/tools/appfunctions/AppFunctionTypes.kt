...
internal object AppFunctionTypes {
  ...
  /** Whether [type] carries a screen this toolset cannot hand to the app, directly or inside it. */
  fun carriesUndeliverablePendingIntent(
    ...
    return when (val resolved = resolve(type, components)) {
      // A bare (or nullable) PendingIntent is the one screen shape this toolset can deliver.
   ...
   * the return value, so it cannot be delivered. The metadata is another app's, so the walk is
