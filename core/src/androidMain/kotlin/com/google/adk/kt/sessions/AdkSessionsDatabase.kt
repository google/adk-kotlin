 ...
 *
 * Holds four tables: `sessions`, `events`, `app_states`, and `user_states`. JSON columns are
 * converted via [JsonConverters].
 *
 * Internal: instances are constructed via [RoomSessionService.fromContext], not directly.
 */
@Database(
  entities = [SessionRow::class, EventRow::class, AppStateRow::class, UserStateRow::class],
  version = 1,
  exportSchema = false,
)
