...
internal constructor(
   ...
   * @param packageNames the apps whose functions are offered, the calling app by default. An empty
   *   set offers nothing. Reaching another app also needs the `EXECUTE_APP_FUNCTIONS` permission,
   *   visibility of that package, and on newer platforms that this caller be allowlisted; short of
   *   that its functions are either invisible here or refused at execution.
   ...
   * @param onUiAction receives the screen a function returned for the app to open. Some app
   *   functions answer with a `PendingIntent` rather than data; without a handler the agent has
   *   nowhere to send it, and such a call reports to the model that it could not be shown. The
   *   handler is retained for the life of the toolset, so it must not capture an activity, and it
   *   runs on whichever dispatcher the agent is on rather than the main thread.
   */
  @JvmOverloads
  constructor(
    context: Context,
    ...
    onUiAction: AppFunctionUiActionHandler? = null,
  ) : this(
  ...
  override suspend fun getTools(readonlyContext: ReadonlyContext?): List<BaseTool> {
    // Every call marshals through AppFunctionData, which does not exist before this level.
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.TIRAMISU) return emptyList()
    // AppFunctionSearchSpec rejects an empty set, where null would mean every package.
  ...
  private suspend fun discoverForInvocation(invocationId: String?): List<BaseTool> {
    ...
    return cacheLock.withLock {
      if (invocationId != cachedInvocationId) {
        cachedTools = discover()
        cachedInvocationId = invocationId
      }
      cachedTools
    }
  }
  ...
  private suspend fun discover(): List<BaseTool> {
    val discovered =
      try {
        ...
        // would abort the whole turn and take every other toolset's tools with it.
        logger.warn(e) { "Could not read the device's app functions." }
        return emptyList()
      }
    ...
    for (metadata in ordered) {
      // `isEnabled` is not filtered on: from API 37 it carries the manifest default rather than
      // the runtime state, so trusting it would hide a function the user has actually enabled.
      // A call to a disabled function returns a DISABLED error, which the model can act on.
      ...
      names.add(name)
      tools.add(AppFunctionTool(metadata, declaration, client, onUiAction))
    }
  ...
  override fun close() {
    cachedInvocationId = null
    cachedTools = emptyList()
  }

  private val cacheLock = Mutex()
  private var cachedInvocationId: String? = null
