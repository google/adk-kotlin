...
internal constructor(
  ...
  private suspend fun discoverForInvocation(invocationId: String?): List<BaseTool> {
    ...
    return cacheLock.withLock {
      ...
      else discover().also { cache = Cache(invocationId, it) }
    }
  }
  ...
  private suspend fun discover(): List<BaseTool> {
    ...
    for (metadata in ordered) {
      ...
      //
      // A screen is for the app to open, not a value the model can read, so a function that
      // answers with one has nothing to report back and is left out rather than offered.
...
internal class PlatformAppFunctionClient(context: Context) : AppFunctionClient {
   ...
   * cannot change while the process lives. The warning rides along, since `lazy` already makes it
   * happen exactly once, and an empty tool list is otherwise indistinguishable from a device that
   * simply declares nothing.
   */
  private val manager: AppFunctionManager? by lazy {
    AppFunctionManager.getInstance(context).also {
      if (it == null) logger.warn { "AppFunctions are unavailable here; offering no tools." }
    }
  }
  ...
  private companion object {
    private val logger = LoggerFactory.getLogger(PlatformAppFunctionClient::class)
  }
}
