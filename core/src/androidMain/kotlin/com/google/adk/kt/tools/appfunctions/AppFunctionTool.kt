...
internal class AppFunctionTool(
  ...
  private val onUiAction: AppFunctionUiActionHandler?,
  ...
  override suspend fun run(context: ToolContext, args: Map<String, Any?>): Any {
    ...
    val response =
      try {
        ...
        // The platform can refuse the call outright rather than answering with an Error.
        logger.warn(e) { "App function $name could not be invoked." }
   ...
   * return something the declared response does not describe.
   */
  private suspend fun readSuccess(response: ExecuteAppFunctionResponse.Success): Map<String, Any?> {
    // Reading the app's value can throw when it does not match the metadata, so it sits inside the
    // same net as the ordinary read rather than escaping and aborting the turn.
    val screen =
      try {
        AppFunctionDataConverter.pendingIntentReturn(metadata, response.returnValue)
      } catch (e: RuntimeException) {
        logger.warn(e) { "App function $name returned a screen that does not match its metadata." }
        return mapOf(ERROR_KEY to "The app function returned a value that could not be read")
      }
    if (screen == null) return readNoScreen(response)
    // The function answered with a screen for the app to open rather than data. Sending it is
    // the app's decision, so it goes to the handler; with none the call cannot finish what it
    // set out to do, and saying otherwise would tell the model the user had seen something.
    val handler =
      onUiAction
        ?: return mapOf(
          ERROR_KEY to
            "The app function returned a screen to open, and no handler is set to show it"
        )
    // A screen opened for a turn the caller has already given up on would be seen by nobody.
    currentCoroutineContext().ensureActive()
    // `PendingIntent.send()` throws once the intent is cancelled, which is the documented way to
    // use this handler, so a failure here has to reach the model rather than end the turn.
    try {
      handler.onUiAction(screen)
    } catch (e: CancellationException) {
      throw e
    } catch (e: Exception) {
      // `CanceledException` is checked, and this one comes from the caller's own handler rather
      // than from the app's value, so it is safe to log with its cause.
      logger.warn(e) { "App function $name produced a screen the app could not open." }
      return mapOf(ERROR_KEY to "The app function returned a screen that could not be opened")
    }
    return mapOf(BaseTool.RESULT_KEY to mapOf(AppFunctionSchemaConverter.UI_ACTION_KEY to true))
  }

  /**
   * Reads a response that produced no screen, telling the reasons for that apart.
   *
   * A function declared to return a screen answers `uiAction = false` rather than an empty value.
   * One that came back with nothing readable, where the declaration puts screens somewhere the
   * handler cannot be given them, says so instead of reporting a bare success.
   */
  private fun readNoScreen(response: ExecuteAppFunctionResponse.Success): Map<String, Any?> {
    val declared = metadata.response.valueType
    if (AppFunctionTypes.isPendingIntent(declared, metadata.components)) {
      return mapOf(BaseTool.RESULT_KEY to mapOf(AppFunctionSchemaConverter.UI_ACTION_KEY to false))
    }
    val value =
      try {
    ...
    // and the screens it could not deliver are dropped like any other unrepresentable property.
    if (
      value.isEmpty() &&
        AppFunctionTypes.carriesUndeliverablePendingIntent(declared, metadata.components)
    ) {
      logger.warn {
        "App function $name returned only screens this toolset cannot hand to the app."
      }
      return mapOf(ERROR_KEY to "The app function returned screens that cannot be opened")
    }
  ...
  private companion object {
     ...
     * from its error code, which the library does not expose.
     */
    private fun AppFunctionException.category(): String =
      when (this) {
