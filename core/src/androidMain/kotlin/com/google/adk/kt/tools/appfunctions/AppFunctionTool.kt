...
internal class AppFunctionTool(
  ...
  /**
   * The function's own method name, logged in place of [name].
   *
   * A tool name is qualified by package whenever more than one package may be offered, so logging
   * it would record which apps are installed; the method alone names the function without the app.
   */
  private val methodName = methodNameOf(metadata.id)
  ...
  override suspend fun run(context: ToolContext, args: Map<String, Any?>): Any {
    val parameters =
      try {
        ...
        // Anything else came from the library rather than the converter, so its message can quote
        // the argument value and must reach neither the model nor the log.
        logger.warn { "App function $methodName could not take the arguments." }
        return mapOf(ERROR_KEY to "The arguments did not fit the function")
      }
    ...
    val response =
      try {
        ...
        if (e is AppFunctionException)
          logger.warn { "App function $methodName failed: ${e.category()}." }
        else logger.warn { "App function $methodName could not be invoked." }
    ...
    return when (response) {
      ...
      is ExecuteAppFunctionResponse.Error -> {
        logger.warn { "App function $methodName failed with ${response.error.category()}." }
  ...
  private fun readSuccess(response: ExecuteAppFunctionResponse.Success): Map<String, Any?> {
    ...
    val value =
      try {
        ...
        // Not only the converter's own `MalformedResponse` lands here, so the throwable's message
        // can quote the app's returned value and is left out.
        logger.warn {
          "App function $methodName returned a value that does not match its metadata."
        }
