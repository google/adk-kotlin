...
internal object AppFunctionSchemaConverter {
  ...
  /** The key a screen-opening result is reported under. */
  const val UI_ACTION_KEY: String = "uiAction"

  /**
   * The response of a function that returns a screen to open rather than data.
   *
   * The `PendingIntent` itself never reaches the model; it goes to the app, and the model is told
   * only that the call opened something.
   */
  private val UI_ACTION_VALUE =
    Schema(type = Type.OBJECT, properties = mapOf(UI_ACTION_KEY to Schema(type = Type.BOOLEAN)))
  ...
  private fun toResponseSchema(metadata: AppFunctionMetadata, scope: RefScope): Schema? {
    val value =
      if (AppFunctionTypes.isPendingIntent(metadata.response.valueType, scope.components)) {
        // A screen still carries whatever the app wrote about what opening it does.
        UI_ACTION_VALUE
      } else {
    ...
    return when (type) {
      is AppFunctionStringTypeMetadata ->
        Schema(
          ...
          // Gemini takes `enum` on a string only alongside this format.
          format = type.enumValues?.let { "enum" },
          nullable = nullable,
        )
  ...
  private fun convertObject(
    ...
    for ((name, property) in properties) {
      ...
      if (schema == null) {
        if (name in requiredNames && property.isNullable != true) return null
        continue
      }
