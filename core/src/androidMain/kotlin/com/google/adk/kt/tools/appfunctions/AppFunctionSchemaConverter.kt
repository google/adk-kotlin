...
internal object AppFunctionSchemaConverter {
  ...
  private class RefScope(
    /** The method name alone, since a function id names the app's package and is not logged. */
    ...
    fun following(ref: String) = RefScope(methodName, components, visited + ref, remaining, members)
  }
    ...
    for (parameter in declared) {
      ...
      if (converted == null) {
        ...
        if (parameter.isRequired && !parameter.dataType.isNullable) {
          logger.warn {
            "Skipping app function ${methodNameOf(metadata.id)}: required parameter " +
              "'${parameter.name}' has no " +
              "schema."
          }
          return null
        }
  ...
  private fun warnDroppedResponse(metadata: AppFunctionMetadata, scope: RefScope): Schema? {
    ...
    if (resolved is AppFunctionParcelableTypeMetadata) {
      // The simple type name: a qualified one carries the package. No explicit default is needed
      // here, unlike for `#`, since a name with no dot has no package to strip.
      logger.warn {
        "App function ${methodNameOf(metadata.id)} returns an unrepresentable parcelable " +
          "'${resolved.qualifiedName.substringAfterLast('.')}'; dropping the response schema."
      }
    }
    return null
  }
  ...
  private fun convert(type: AppFunctionDataTypeMetadata, depth: Int, scope: RefScope): Schema? {
    if (depth >= MAX_SCHEMA_DEPTH) {
      logger.warn {
        "App function ${scope.methodName} schema nests deeper than $MAX_SCHEMA_DEPTH levels; " +
          "dropping it."
      }
      return null
    }
  ...
  private fun convertReference(
    ...
    if (ref in scope.visited) {
      logger.warn {
        "App function ${scope.methodName} schema references '$ref' from itself; dropping the " +
          "recursive property."
      }
      return null
    }
    ...
    if (target == null) {
      logger.warn {
        "App function ${scope.methodName} schema has a reference '$ref' that resolves to nothing; " +
          "dropping it."
      }
      return null
    }
    ...
    if (resolved == null) {
      logger.warn {
        "App function ${scope.methodName} schema has a reference chain from '$ref' that does not " +
          "resolve; dropping it."
      }
      return null
    }
    ...
    if (resolved !is AppFunctionObjectTypeMetadata && resolved !is AppFunctionAllOfTypeMetadata) {
      logger.warn {
        "App function ${scope.methodName} schema references non-object type '$ref'; dropping the " +
          "property."
      }
      return null
    }
    if (!scope.spend()) {
      logger.warn {
        "App function ${scope.methodName} schema expands more than $MAX_REF_EXPANSIONS references; " +
          "dropping the rest."
      }
      return null
    }
...
/**
 * The method part of an AppFunction [id], which names the function without naming the app.
 *
 * An id joins a qualified class name and a method with `#`, so the package sits before the
 * separator and must never reach a log. The delimiter is not guaranteed, and the default
 * `substringAfterLast` would then return the whole id, so the empty case is spelled out.
 */
internal fun methodNameOf(id: String): String =
  id.substringAfterLast('#', missingDelimiterValue = "").ifEmpty { "(unnamed)" }
