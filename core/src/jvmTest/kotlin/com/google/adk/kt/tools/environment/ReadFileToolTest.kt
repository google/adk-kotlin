...
class ReadFileToolTest {
  ...
  private class StubEnvironment(private val files: Map<String, ByteArray>) : BaseEnvironment {
    val executeCalls = mutableListOf<String>()
    override val workingDir: String = "/tmp/adk-test"
  ...
  fun readFileWithLineRange_returnsSelectedLines() = runBlocking {
    env.writeFile("sample.txt", "line1\nline2\nline3\n".encodeToByteArray()).getOrThrow()
  ...
  fun readFile_splitsOnBareCarriageReturn() = runBlocking {
    ...
    env.writeFile("mac.txt", "a\rb\rc".encodeToByteArray()).getOrThrow()
  ...
  fun readFile_emptyFile_reportsZeroLines() = runBlocking {
    env.writeFile("empty.txt", ByteArray(0)).getOrThrow()
