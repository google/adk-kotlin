...
class EditFileToolTest {
  ...
  fun editFile_handlesLineBreaks_linuxFileWindowsSearch() = runBlocking {
    env.writeFile("test.txt", "line1\nline2\nline3".encodeToByteArray()).getOrThrow()
    ...
    assertEquals(
      "line1_replaced\nline2_replaced\nline3",
      env.readFile("test.txt").getOrThrow().decodeToString(),
    )
  }
  ...
  fun editFile_failsOnMultipleMatches() = runBlocking {
    env.writeFile("test.txt", "line1\nline2\nline1\nline2".encodeToByteArray()).getOrThrow()
  ...
  fun editFile_emptyOldString_returnsError() = runBlocking {
    env.writeFile("test.txt", "line1".encodeToByteArray()).getOrThrow()
  ...
  fun editFile_exactMatchWorks() = runBlocking {
    ...
    assertEquals("ok", result["status"])
    assertEquals("replaced\nline3", env.readFile("test.txt").getOrThrow().decodeToString())
  }

  @Test
  fun editFile_handlesSpecialRegexChars() = runBlocking {
    env.writeFile("test.txt", "line1.content\nline2".encodeToByteArray()).getOrThrow()
    ...
    assertEquals("ok", result["status"])
    assertEquals("replaced\nline2", env.readFile("test.txt").getOrThrow().decodeToString())
  }
}
