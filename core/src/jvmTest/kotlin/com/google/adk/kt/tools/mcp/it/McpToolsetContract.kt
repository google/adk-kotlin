...
class McpToolsetContract(private val harness: McpToolsetHarness) {
  ...
  suspend fun getTools_withUseMcpResources_appendsResourceTools() =
    harness.withToolset(useMcpResources = true) { toolset ->
      // The resource tools are appended only because the live server advertises the resources
  ...
  suspend fun listResources_returnsEntryCarryingNameAndUri() =
    harness.withToolset(useMcpResources = false) { toolset ->
      // A real resources/list round-trip: the typed entry carries both the human-readable name and
      // the canonical uri, and that uri is exactly the identifier readResource takes.
      val entry =
  ...
  private companion object {
    ...
    private val RESOURCE_TOOLS = arrayOf("list_mcp_resources", "load_mcp_resource")
  }
}
