...
class McpToolTest {
  ...
  fun declaration_failingConversion_throwsEveryTime() {
    ...
    val tool =
      newTool(McpSchema.Tool.builder("badTool", badSchema).build())
  ...
  @Test
  fun run_rethrowsCoreFailureCause() = runTest {
    val tool =
    ...
    val error =
      assertFailsWith<IllegalStateException> { tool.run(toolContext, emptyMap()) }
  ...
  fun mcpSchemaConverter_convertsMcpSchemaToolToAdkFunctionDeclaration() {
    val mcpInputSchema: Map<String, Any> =
      mapOf(
        "type" to "object",
        "properties" to mapOf("param1" to mapOf("type" to "string", "description" to "param1 description")),
