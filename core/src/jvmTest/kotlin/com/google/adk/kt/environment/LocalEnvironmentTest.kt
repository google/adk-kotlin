...
class LocalEnvironmentTest {
  ...
  fun setUp() {
    ...
    runBlocking { env.initialize(readonlyContext) }
  }
  ...
  fun execute_mergesEnvVars(): Unit = runBlocking {
    ...
    withVar.initialize(readonlyContext)
  ...
  fun execute_inheritedEnvVarAllowlist_dropsUnlistedHostVars(): Unit = runBlocking {
    ...
    restricted.initialize(readonlyContext)
  ...
  fun execute_beforeInitialize_withProvidedWorkingDir_throwsIllegalState(): Unit = runBlocking {
    // Even a provided, already-existing directory must not be usable until initialize() has run:
    // the workspace is only resolved by initialize(), never seeded from the constructor.
  ...
  fun execute_launchFailure_returnsEnvironmentException(): Unit = runBlocking {
    // Delete the working directory after initialize so ProcessBuilder.start() cannot launch, which
    // exercises execute()'s IOException -> EnvironmentException wrap.
  ...
  fun readFile_directory_returnsFailure(): Unit = runBlocking {
    ...
    assertTrue(result.isFailure)
    assertTrue(
      (result.exceptionOrNull() as? EnvironmentException)
        ?.message
  ...
  fun initialize_uncreatableWorkingDir_throwsEnvironmentException(): Unit = runBlocking {
    // A regular file where a parent directory is expected makes createDirectories fail, exercising
    // the IOException -> EnvironmentException wrap in initialize().
    val file = root.resolve("a-file")
    Files.writeString(file, "x")
    val environment = LocalEnvironment(file.resolve("sub").toString())

    assertFailsWith<EnvironmentException> { environment.initialize(readonlyContext) }
  }
}
