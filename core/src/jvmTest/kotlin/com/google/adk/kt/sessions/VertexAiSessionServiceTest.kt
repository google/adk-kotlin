...
class VertexAiSessionServiceTest {
  ...
  @Test
  fun appendEvent_clientFails_leavesSessionUnmutated() = runTest {
    val client =
      mock<VertexAiSessionsClient> {
        onBlocking { appendEvent(any(), any(), any()) } doReturn
          Result.failure(IOException("append failed"))
    ...
    // A failed remote append must not mutate the caller's in-memory session.
    assertThat(session.events).isEmpty()
    assertThat(session.state.containsKey("k")).isFalse()
  }
