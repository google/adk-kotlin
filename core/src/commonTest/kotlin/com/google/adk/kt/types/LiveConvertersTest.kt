...
class LiveConvertersTest {
  ...
  fun liveConnectConfig_mirrorsEverySdkFieldItDoesNotDeliberatelyDrop() {
    // Names only: a field mapped onto the wrong target still passes here, and the round trip
    // above is what catches that. This catches an SDK field nobody has decided about yet.
    // Kept local rather than shared with the case above: that one asserts on properties, and
    // rewriting it to read a list collides with work in flight on this file.
  ...
  fun fullyPopulatedLiveConnectConfig_setsEveryDeclaredField() {
    // The round trips above cover only what the fixture populates, so a field added to
    // `LiveConnectConfig` and not to the fixture would leave coverage without failing.
    // Encoding omits anything still at its default, so a missing key is a missing field.
