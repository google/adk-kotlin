...
class ResumableLlmAgentTest {
  ...
  private fun sequentialModel(tag: String, vararg textResponses: String): DummyModel =
  ...
  private fun someTool(name: String = "some_tool"): DummyTool =
    DummyTool(name = name, onRun = { _, _ -> mapOf("result" to "ok") })

  private fun baseAgentState(): TypedData.MapValue = TypedData.MapValue(emptyMap())

  private fun modelEvent(author: String, part: Part): Event =
    Event(
      author = author,
      invocationId = INVOCATION_ID,
  ...
  private fun toolCallPart(toolName: String): Part =
  ...
  private fun toolResponsePart(toolName: String): Part =
  ...
  private fun toolResponsePartWithId(toolName: String): Part =
    Part(
      functionResponse =
        FunctionResponse(name = toolName, id = "${toolName}_id", response = mapOf("result" to "ok"))
    )

  private suspend fun resumableContext(
    agent: BaseAgent,
    ...
    return InvocationContext(
      session = sessionService.getSession(key)!!,
      runConfig = null,
      agent = agent,
      invocationId = INVOCATION_ID,
  ...
  private suspend fun resumeAndSimplify(
    agent: BaseAgent,
