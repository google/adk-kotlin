...
class VideoMetadataWireFormatTest {
  ...
  /**
   * `"Potato"` above does not end in `s`, so the SDK rejects it before parsing any seconds. These
   * two reach `String.toDouble()`, whose `NumberFormatException` is a sibling of
   * [SerializationException] rather than an instance of it, and so would otherwise slip past the
   * `catch (SerializationException)` sites around decoding.
   */
  @Test
  fun videoMetadata_decodedFromNonNumericSecondsString_failsAsASerializationError() {
    assertFailsWith<SerializationException> {
      json.decodeFromString(VideoMetadata.serializer(), """{"startOffset":"5xs"}""")
    }
  }

  @Test
  fun videoMetadata_decodedFromMalformedIsoString_failsAsASerializationError() {
    assertFailsWith<SerializationException> {
      json.decodeFromString(VideoMetadata.serializer(), """{"startOffset":"Ps"}""")
    }
  }
