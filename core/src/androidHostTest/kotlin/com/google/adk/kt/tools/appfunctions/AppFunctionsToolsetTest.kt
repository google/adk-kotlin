...
class AppFunctionsToolsetTest {
  ...
  @Test
  fun getTools_disabledFunction_isStillOffered() =
    runBlocking<Unit> {
      // From API 37 `isEnabled` carries the manifest default rather than the runtime state, so
      // filtering on it would hide a function the user has enabled. Calling a genuinely disabled
      // one returns a DISABLED error, which the model can act on.
      val client = FakeAppFunctionClient(listOf(function(enabled = false)))
  ...
  @Test
  fun run_screenReturnWhoseValueIsNotAScreen_reportsItCannotBeRead() =
    runBlocking<Unit> {
      // Declared as a screen but answered with text: the read of the intent itself fails, which is
      // a different path from a mismatch on an ordinary value.
      val declaredAsScreen = function(response = pendingIntentType())
      val actuallyText = function(response = AppFunctionStringTypeMetadata(isNullable = false))
      val client =
        FakeAppFunctionClient(listOf(declaredAsScreen)) {
          ExecuteAppFunctionResponse.Success(
  ...
  @Test
  fun getTools_functionReturningAScreen_declaresTheUiActionResponse() =
    runBlocking<Unit> {
      val client = FakeAppFunctionClient(listOf(function(response = pendingIntentType())))

      val response = toolset(client).getTools().single().declaration()?.response

      // The PendingIntent itself never reaches the model; it is told only that a screen opened.
      assertThat(response?.properties?.get("result")?.properties?.keys).containsExactly("uiAction")
    }

  @Test
  fun run_functionReturningAScreen_handsItToTheHandler() =
    runBlocking<Unit> {
      val metadata = function(response = pendingIntentType())
      val screen = testPendingIntent()
      val client =
        FakeAppFunctionClient(listOf(metadata)) {
          ExecuteAppFunctionResponse.Success(
            AppFunctionData.Builder(metadata.response, metadata.components)
              .setParcelable(ExecuteAppFunctionResponse.Success.PROPERTY_RETURN_VALUE, screen)
              .build()
          )
        }
      var handed: PendingIntent? = null

      val result =
        toolset(client, onUiAction = { handed = it })
          .getTools()
          .single()
          .run(testToolContext(), emptyMap())

      assertThat(handed).isEqualTo(screen)
      assertThat(result).isEqualTo(mapOf("result" to mapOf("uiAction" to true)))
    }

  @Test
  fun run_functionReturningAScreen_withNoHandler_reportsItCannotBeShown() =
    runBlocking<Unit> {
      val metadata = function(response = pendingIntentType())
      val client =
        FakeAppFunctionClient(listOf(metadata)) {
      ...
      // Claiming success would tell the model the user had seen a screen that was never opened.
      @Suppress("UNCHECKED_CAST") val error = (result as Map<String, Any?>)["error"] as? String
      assertThat(error).contains("no handler")
    }

  @Test
  fun run_uiActionHandlerThrows_reportsItInsteadOfEndingTheTurn() =
    runBlocking<Unit> {
      // `PendingIntent.send()` throws once the intent is cancelled, and that is the documented way
      // to use the handler, so it must not escape into the agent's flow.
      val metadata = function(response = pendingIntentType())
      val client =
        FakeAppFunctionClient(listOf(metadata)) {
          ExecuteAppFunctionResponse.Success(
      ...
      // The exception `PendingIntent.send()` actually throws, and a checked one, so a guard on
      // `RuntimeException` would let it past and end the whole turn.
      val result =
        toolset(client, onUiAction = { throw PendingIntent.CanceledException() })
          .getTools()
          .single()
          .run(testToolContext(), emptyMap())

      @Suppress("UNCHECKED_CAST") val error = (result as Map<String, Any?>)["error"] as? String
      assertThat(error).contains("could not be opened")
    }

  @Test
  fun run_uiActionHandlerIsCancelled_endsTheTurnInsteadOfReporting() =
    runBlocking<Unit> {
      val metadata = function(response = pendingIntentType())
      val client = FakeAppFunctionClient(listOf(metadata)) { screenResponse(metadata) }

      val tool =
        toolset(client, onUiAction = { throw CancellationException("gave up") }).getTools().single()

      val thrown = runCatching { tool.run(testToolContext(), emptyMap()) }.exceptionOrNull()

      assertThat(thrown).isInstanceOf(CancellationException::class.java)
    }

  @Test
  fun run_nullableScreenTheAppLeftUnset_reportsThatNoScreenOpened() =
    runBlocking<Unit> {
      // The declared response promises `uiAction`, so an empty success would contradict it.
      val metadata = function(response = pendingIntentType(isNullable = true))
      val client =
        FakeAppFunctionClient(listOf(metadata)) {
          ExecuteAppFunctionResponse.Success(
            AppFunctionData.Builder(metadata.response, metadata.components).build()
          )
        }

      val result =
        toolset(client, onUiAction = {}).getTools().single().run(testToolContext(), emptyMap())

      @Suppress("UNCHECKED_CAST") val value = (result as Map<String, Any?>)[BaseTool.RESULT_KEY]
      assertThat(value).isEqualTo(mapOf(AppFunctionSchemaConverter.UI_ACTION_KEY to false))
    }

  @Test
  fun run_functionReturningSeveralScreens_reportsItCannotOpenThem() =
    runBlocking<Unit> {
      // The handler takes one intent, so a list of them has nowhere to go; saying so beats the
      // empty success an ordinary read would produce.
      ...
      @Suppress("UNCHECKED_CAST") val error = (result as Map<String, Any?>)["error"] as? String
      assertThat(error).contains("cannot be opened")
    }
  ...
  @Test
  fun getTools_nonScreenParcelableReturn_isNotTreatedAsAUiAction() =
    runBlocking<Unit> {
      // Proves the discriminator discriminates rather than matching any parcelable.
      val other =
  ...
  private companion object {
    fun toolset(
      ...
      onUiAction: ((PendingIntent) -> Unit)? = null,
    ) = AppFunctionsToolset(client, packageNames, toolFilter, onUiAction)
    ...
    /** A success carrying one screen, the shape the handler is given. */
    fun screenResponse(metadata: AppFunctionMetadata) =
      ExecuteAppFunctionResponse.Success(
        AppFunctionData.Builder(metadata.response, metadata.components)
          .setParcelable(
            ExecuteAppFunctionResponse.Success.PROPERTY_RETURN_VALUE,
