...
class AppFunctionDataConverterTest {
  ...
  @Test
  fun pendingIntentReturn_pendingIntentResponse_handsBackTheIntent() {
    val intent = pendingIntent()
    val metadata = metadata(response = pendingIntentType())
    val returnValue = returnValue(metadata) { setParcelable(RETURN_KEY, intent) }

    assertThat(AppFunctionDataConverter.pendingIntentReturn(metadata, returnValue))
      .isEqualTo(intent)
  }

  @Test
  fun pendingIntentReturn_responseThatIsNotAScreen_returnsNull() {
    val metadata = metadata(response = string())
    val returnValue = returnValue(metadata) { setString(RETURN_KEY, "Groceries") }

    assertThat(AppFunctionDataConverter.pendingIntentReturn(metadata, returnValue)).isNull()
  }

  @Test
  fun pendingIntentReturn_nullablePendingIntentTheAppLeftUnset_returnsNull() {
    // Indistinguishable from "not a screen at all" here, which is why the tool reports it rather
    // than falling through to an ordinary read.
    val metadata = metadata(response = pendingIntentType(isNullable = true))
    val returnValue = returnValue(metadata) {}
