...
class AppFunctionDataConverterTest {
  ...
  @Test
  fun toAppFunctionData_largeDecimalDouble_isAccepted() {
    // The magnitude check applies to integral types only: `1e300` has no exact whole form, and
    // `toLong` would saturate it onto `Long.MAX_VALUE` and reject a value a double holds fine.
    val metadata = metadata(param("ratio", AppFunctionDoubleTypeMetadata(isNullable = false)))

    val data = AppFunctionDataConverter.toAppFunctionData(metadata, mapOf("ratio" to 1e300))

    assertThat(data.getDouble("ratio")).isEqualTo(1e300)
  }
