...
class AppFunctionSchemaConverterTest {
  ...
  @Test
  fun toFunctionDeclaration_stringFormatDateTime_carriesIt() {
    val declaration =
      convert(
        params =
          listOf(
            param("due", AppFunctionStringTypeMetadata(isNullable = false, format = "date-time"))
          )
      )

    val due = checkNotNull(declaration?.parameters?.properties?.get("due"))
    assertThat(due.format).isEqualTo("date-time")
  }

  @Test
  fun toFunctionDeclaration_stringEnumWithAnotherFormat_keepsTheEnumFormat() {
    // Gemini needs "enum" alongside the values; the app's own format cannot displace it.
    val declaration =
      convert(
        params =
          listOf(
  ...
  fun toFunctionDeclaration_deeplyNestedType_dropsFunctionRatherThanRecursingForever() {
    // Deeper than the converter's nesting cap, and each level is required. Nested objects rather
    // than arrays, so the cap is what drops it and not the array-of-array refusal.
    var type: AppFunctionDataTypeMetadata = string()
    repeat(40) {
      type =
        AppFunctionObjectTypeMetadata(
          properties = mapOf("inner" to type),
          required = listOf("inner"),
          qualifiedName = "com.example.Deep",
          isNullable = false,
        )
    }
  ...
  @Test
  fun toFunctionDeclaration_longReferenceChain_dropsFunctionRatherThanFollowingForever() {
    // Each hop resolves to the next and the last to a string, so nothing but the depth cap stops
    // the walk -- which is what pins the increment in `convertReference`.
    val hops = 40
    val chain =
      (0 until hops).associate { "Ref$it" to reference("Ref${it + 1}") } +
        ("Ref$hops" to note() as AppFunctionDataTypeMetadata)

    val declaration =
      convert(
        params = listOf(param("deep", reference("Ref0"), required = true)),
        components = AppFunctionComponentsMetadata(dataTypes = chain),
      )
  ...
  @Test
  fun toFunctionDeclaration_arrayOfArrays_dropsTheParameter() {
    // The data side has no setter for a nested array, so offering one would fail every call.
    val nested =
      AppFunctionArrayTypeMetadata(
        itemType = AppFunctionArrayTypeMetadata(itemType = string(), isNullable = false),
        isNullable = false,
      )

    assertThat(convert(params = listOf(param("rows", nested, required = true)))).isNull()
  }
  ...
  @Test
  fun toFunctionDeclaration_screenReturn_describesNoResponse() {
    // A screen is a parcelable the model cannot read, so it is dropped like any other. The toolset
    // does not offer such a function at all; this pins what the converter alone does with one.
  ...
  @Test
  fun methodNameOf_idWithSeparator_returnsTheMethodAlone() {
    assertThat(methodNameOf(FUNCTION_ID)).isEqualTo("createNote")
  }

  @Test
  fun methodNameOf_idWithoutSeparator_readsAsUnnamedRatherThanTheWholeId() {
    // `substringAfterLast` returns the whole string when the delimiter is absent, so without the
    // explicit empty default this would put the app's package into a log.
    val noSeparator = "com.example.notes.NotesFunctions"

    assertThat(methodNameOf(noSeparator)).isEqualTo("(unnamed)")
    assertThat(methodNameOf(noSeparator)).doesNotContain("com.example")
  }
