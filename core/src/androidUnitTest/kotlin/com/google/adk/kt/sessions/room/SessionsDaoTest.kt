...
class SessionsDaoTest {
  ...
  fun eachTable_insertAndRead_roundTrips() = runTest {
    val sessionRow =
      SessionRow(
        ...
        id = "session1",
        state = emptyMap(),
        createTimeEpochMs = 1L,
        updateTimeEpochMs = 1L,
      )
    val appStateRow = AppStateRow(appName = "app", state = emptyMap(), updateTimeEpochMs = 1L)
    val userStateRow =
      UserStateRow(appName = "app", userId = "user", state = emptyMap(), updateTimeEpochMs = 1L)
    val eventRow =
      EventRow(
        id = "event1",
        appName = "app",
  ...
  fun appendEventAtomic_existingSession_persistsAndMerges() = runTest {
    ...
    dao.insertSession(
      SessionRow(
        ...
        createTimeEpochMs = initialTs,
        updateTimeEpochMs = initialTs,
      )
    )
    dao.insertAppStateIfAbsent(
      AppStateRow(appName = "app", state = emptyMap(), updateTimeEpochMs = initialTs)
    )
    ...
    assertThat(updatedSession.updateTimeEpochMs).isEqualTo(eventTs)
  ...
  fun appendEventAtomic_noSessionDelta_touchesUpdateTimeAndKeepsState() = runTest {
    ...
    // With no session delta the state is unchanged, but update_time is bumped (touchSession
    // branch).
    val updatedSession = dao.getSession("app", "user", "session1")!!
  ...
  fun appendEventAtomic_staleUpdateTime_throws() = runTest {
    ...
    dao.insertSession(
      SessionRow(
        ...
        id = "session1",
        state = emptyMap(),
        createTimeEpochMs = storedTs,
        updateTimeEpochMs = storedTs,
      )
    )
