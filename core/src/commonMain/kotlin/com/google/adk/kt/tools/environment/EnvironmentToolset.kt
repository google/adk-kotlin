 ...
 * [maxOutputChars].
 *
 * Lifecycle: [close] closes the [environment] once, if it was initialized. Because [Toolset]'s
 * [close] is not a suspend function, it bridges to the suspend [BaseEnvironment.close] via
 * `runBlocking`, so call it from a shutdown / non-coroutine context (not from within a coroutine).
 *
...
class EnvironmentToolset(
  ...
  override suspend fun processLlmRequest(
    ...
    ensureInitialized()
    val instruction = environmentInstruction(environment.workingDir)
    return llmRequest.appendInstructions(Content(parts = listOf(Part(text = instruction))))
  }

  override fun close() {
    if (environmentInitialized) {
      runBlocking { environment.close() }
