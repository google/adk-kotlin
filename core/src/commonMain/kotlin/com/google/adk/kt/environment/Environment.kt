...
interface Environment : AutoCloseable {
   ...
   *
   * This method is not `suspend`, so that an environment can be closed from the same places as the
   * rest of the ADK's [AutoCloseable] types, such as a [com.google.adk.kt.tools.Toolset]. An
   * implementation whose teardown is asynchronous either bridges to it here or, if waiting is not
   * required, starts it and returns.
   */
  override fun close() {}
