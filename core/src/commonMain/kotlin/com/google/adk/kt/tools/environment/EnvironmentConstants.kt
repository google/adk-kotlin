 ...
 * tool-selection rules. The workspace is not named by path: a multi-tenant environment has a
 * separate directory per session, so there is no single path to report here.
 */
internal fun environmentInstruction(): String =
  """
  ...
  - Touch one file from two calls in the same response (they run in parallel, so a read can miss a write, and one of two writes is lost); finish with a file before using it again
  """
    .trimIndent()
...
internal fun Throwable.toEnvironmentErrorResponse(logger: Logger): Map<String, Any> {
  ...
  logger.warn(this) {
    "Environment returned Result.failure wrapping an unrecognized exception type " +
