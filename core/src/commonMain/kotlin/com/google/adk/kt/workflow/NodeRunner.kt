...
internal class NodeRunner(
  ...
  suspend fun run(nodeInput: Any?): NodeContext {
    var attempt = 1
    while (true) {
      val context = newContext(attempt)
      try {
        runAttempt(context, nodeInput)
        ...
        // This node's own timeout already arrived as a NodeTimeoutException, so a cancellation
        // reaching here is the scope being torn down around it and must not be retried.
        throw e
      } catch (e: Exception) {
        // Every failed attempt is reported, so a retried node's history shows each try. The author
        // is left empty here and assigned by stamp, the single place events are attributed.
        ...
        val policy = node.retryConfig
        if (policy == null || !policy.shouldRetry(e, attempt)) {
          context.failure = NodeExecutionFailure(e, context.nodePath)
          return context
        }
  ...
  private suspend fun runAttempt(context: NodeContext, nodeInput: Any?) {
    val timeout = node.timeout
    if (timeout == null) {
      runAndDispatchEvents(context, nodeInput)
      return
    }
    try {
      withTimeout(timeout) { runAndDispatchEvents(context, nodeInput) }
    } catch (e: TimeoutCancellationException) {
      throw NodeTimeoutException(node.name, timeout, e)
    }
  }
