...
class LocalEnvironment(
  ...
  /**
   * What the caller requested; null or empty means "create a temporary workspace on initialize".
   */
  private val requestedWorkingDir: String? = workingDir

  /**
   * The resolved workspace, set once by [initialize] and cleared by [close]. It is `null` until
   * [initialize] runs, so any use before then is rejected whether or not a directory was passed to
   * the constructor.
   */
  private var resolvedWorkingDir: String? = null
  ...
  override suspend fun initialize(context: ReadonlyContext?) {
    withContext(Dispatchers.IO) {
      synchronized(lifecycleLock) {
        if (resolvedWorkingDir != null) return@synchronized // already initialized; reuse it.
        try {
          if (requestedWorkingDir.isNullOrEmpty()) {
            ...
            Files.createDirectories(Paths.get(requestedWorkingDir))
            resolvedWorkingDir = requestedWorkingDir
          }
        } catch (e: IOException) {
  ...
  override fun close() {
    synchronized(lifecycleLock) {
      if (autoCreated && resolvedWorkingDir != null) {
        deleteWorkspace(Paths.get(resolvedWorkingDir))
   ...
   *
   * Deletion is best-effort: [close] is called while tearing things down, so a failure to remove a
   * file is logged rather than raised. Both the success and the failure are logged here, so all
   * workspace-teardown logging lives in one place.
   */
  ...
  override suspend fun execute(
    ...
    return try {
      ...
      // Delete each independently so a failure on the first still attempts the second.
      try {
    ...
    withContext(Dispatchers.IO) {
      runCatchingEnv {
        ...
        if (!Files.isRegularFile(target)) {
          throw EnvironmentException("Not a regular file: $path")
        }
        try {
  ...
  private fun resolve(path: String): Path {
    ...
    // Lexically normalized (`.`/`..` collapsed) but with symlinks unresolved; the real, link-
    // resolved path is computed below.
    val normalizedPath =
    ...
    val ancestor = deepestExistingAncestor(normalizedPath, path)
    val resolved =
      try {
        ancestor.toRealPath().resolve(ancestor.relativize(normalizedPath))
      } catch (e: IOException) {
  ...
  /**
   * Returns the deepest ancestor of [normalizedPath] (including itself) that exists on disk,
   * without following links. That is the longest prefix whose links [Path.toRealPath] can resolve;
   * the remainder does not exist yet (e.g. a [writeFile] to a new path).
   *
   ...
   */
  private fun deepestExistingAncestor(normalizedPath: Path, originalPath: String): Path {
    var existing: Path = normalizedPath
