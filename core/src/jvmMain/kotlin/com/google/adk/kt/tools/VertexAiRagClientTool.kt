 ...
 *
 * Unlike [VertexAiRagRetrieval], which grounds the model natively via `vertexRagStore`, this tool
 * runs the retrieval itself: [run] queries the Vertex AI RAG service through a [VertexAiRagClient]
 * and returns the matching context texts, so it works on any model as an ordinary function tool.
 * The corpus and [vectorDistanceThreshold] are fixed per instance; the model supplies `query` and
 * an optional `similarityTopK` per call.
 */
...
private constructor(
   ...
   * @param httpClient The ktor [HttpClient]. The tool takes ownership: it closes the client when
   *   the tool is closed, or closes it and rethrows if the arguments are invalid, so pass a client
  ...
  constructor(
    ...
    // Validate before wrapping httpClient, closing it if the corpus is invalid.
    corpusName =
      closingHttpClientOnFailure(httpClient) {
        normalizeRagCorpusName(ragCorpus, project, location)
      },
  ...
  override suspend fun run(context: ToolContext, args: Map<String, Any?>): Any {
    ...
    val topKArg = args["similarityTopK"]
    val similarityTopK =
      when (topKArg) {
        null -> null
        is Number -> topKArg.toInt()
        else -> return errorResult("'similarityTopK' must be an integer.")
      }
    if (similarityTopK != null && similarityTopK <= 0) {
      return errorResult("'similarityTopK' must be positive.")
    }
  ...
  class Builder {
    ...
    fun build(): VertexAiRagClientTool =
      VertexAiRagClientTool(
        name = checkNotNull(name) { "VertexAiRagClientTool.Builder requires name to be set." },
        description =
          checkNotNull(description) {
            "VertexAiRagClientTool.Builder requires description to be set."
          },
        project =
          checkNotNull(project) { "VertexAiRagClientTool.Builder requires project to be set." },
        location =
          checkNotNull(location) { "VertexAiRagClientTool.Builder requires location to be set." },
        ragCorpus =
          checkNotNull(ragCorpus) { "VertexAiRagClientTool.Builder requires ragCorpus to be set." },
        vectorDistanceThreshold = vectorDistanceThreshold,
  ...
  companion object {
    ...
    private fun errorResult(message: String): Map<String, String> =
      mapOf("error" to message, "error_code" to "INVALID_ARGUMENTS")
  }
}
